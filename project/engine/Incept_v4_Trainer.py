...
from project.engine.utils.vector_file_handler import VectorSaver
from project.engine.utils.vector_file_handler import establish_vectors_folder
from project.engine.utils import configs
from project.engine.utils.ops import walk
...
class Incept_v4_Trainer(object):
    def __init__(self, image_dir, output_graph, output_labels, summaries_dir, how_many_training_steps,
            learning_rate, testing_percentage, eval_step_interval, train_batch_size, vector_path,
            test_batch_size, validation_batch_size, print_misclassified_test_images, model_dir, bottleneck_dir,
            final_tensor_name, flip_left_right, random_crop, random_scale, random_brightness, check_point_path,
            max_ckpts_to_keep, gpu_list, validation_percentage):
            ...
            with tf.device(gpu):
                ...
                for i in range(self.how_many_training_steps):
                    ...
                    if (i % self.eval_step_interval) == 0 or is_last_step:
                        ...
                        _ = saver.save(sess, os.path.join(self.check_point_path, configs.I4_MODEL_CHECKPOINT_NAME.format(i)))
        ...
        for gpu in gpu_list:
            with tf.device(gpu):
                with tf.Session(config=config) as sess:
                    ...
                    vectors_path = establish_vectors_folder(self.vector_path, False, model_type='iv4')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_graph', default='graph/output_graph.pb')
    parser.add_argument('--output_labels', default='graph/output_labels.txt')
    parser.add_argument('--summaries_dir', default='/tmp/retrain_logs')
    parser.add_argument('--how_many_training_steps', default=100)
    parser.add_argument('--learning_rate', default=0.001)
    parser.add_argument('--testing_percentage', default=10)
    parser.add_argument('--validation_percentage', default=10)
    parser.add_argument('--eval_step_interval', default=10)
    parser.add_argument('--train_batch_size', default=32)
    parser.add_argument('--test_batch_size', default=-1, help='-1 means All images')
    parser.add_argument('--validation_batch_size', default=32)
    parser.add_argument('--print_misclassified_test_images', default=False)
    parser.add_argument('--model_dir', default='models')
    parser.add_argument('--bottleneck_dir', default='bottleneck')
    ...
    parser.add_argument('--vector_path', default='vectors')

    parser.add_argument('--check_point_path', default='check_point')
    parser.add_argument('--max_ckpts_to_keep', default=5)
    parser.add_argument('--gpu_list', default=['/gpu:0'])
    ...
    t4 = Incept_v4_Trainer(image_dir=ARGS.image_dir, output_graph=ARGS.output_graph, output_labels=ARGS.output_labels,
                           ...
                           validation_batch_size=ARGS.validation_batch_size, vector_path=ARGS.vector_path,
    ...
    vector_actual_path = t4.vectorize_with_GPU(gpu_list=['/gpu:0'])
